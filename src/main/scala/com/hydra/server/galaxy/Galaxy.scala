...
case class GalaxyWithPlayers(name: String, planets: List[PlanetWithPlayer], timer: Int, players: List[Player], winner: Option[Player]) {
  private val TimerIncreaseRate = 1

  def update: GalaxyWithPlayers = GalaxyWithPlayers(name, planets.map(_.update), timer + TimerIncreaseRate, players, winner)
}